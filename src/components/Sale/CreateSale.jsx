...
const Sale = () => {
  ...
  const addToSaleList = async () => {
    ...
    const totalSaledItem = selectedItems.reduce((sum, item) => sum + parseInt(item.quantity), 0);

    const updatedSaleData = {
      companyId: decode.companyId,
      customerId: customerData[0]._id,
      paymentMode: "Cash",
      saleItemList: itemList,
      totalSaledItem: totalSaledItem,
      saleGrossAmount: gstAmountTotal().toFixed(2),
      saleCGSTAmount: cgstAmountTotal().toFixed(2),
      saleSGSTAmount: sgstAmountTotal().toFixed(2),
      saleNetAmount: selectedItems.reduce((total, item) => total + item.totalAmount, 0),
    };
    ...
    try {
      const resp = await axios.post('http://localhost:4000/s/sale', updatedSaleData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  console.log("pruchase data",saleData);
  // console.log("customer data", customerData[0]._id);

  const handlePrint = () => {
    // Use navigate function to navigate and pass saleData as state
    navigate('/salepdf', { state: { saleData: saleData, itemData: selectedItems } });
  };
    ...
    <>
      ...
      <div className="ml-64 flex px-2 gap-4">
        <div className="bg-darkGreen px-2 w-9/12 h-fit">
          <h1 className="text-3xl text-white text-center p-4">Create Sale</h1>
          <div>
        ...
        <div className="bg-deeperGreen fixed right-4 py-4 px-6 w-60 h-fit mt-4">
          ...
          <div className="flex flex-col">
            ...
            <button onClick={addToSaleList} className="bg-primary hover:opacity-80 mt-4 h-12 font-bold">Save</button>
